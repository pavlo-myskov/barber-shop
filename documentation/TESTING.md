...
|The form section was slightly overlapping by the sticky navbar on small screen sizes after moving to the section using *Book Now* link|The separate div was added to the parent section, set an absolute position for it within the parent section and moved it higher just by the size of the navbar and than hid it.
|Images of the detail-service section are part of the main content|Backround images were replased with an *image tag* with *alt attribute* as this is SEO best practice and they will be accessible for screen readers by the *alt* attribute|
